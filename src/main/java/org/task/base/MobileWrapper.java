...
import org.task.utils.Reporter;
...
public class MobileWrapper extends Reporter{
	...
	public boolean launchApp(String appPackage, String appActivity, String deviceName,String udid,String platformVersion) {
		try {
	...
	public long takeScreenshot() {
		...
		try {
			...
			File descFile = new File("./screenshots/"+number+".png");
	...
	protected List<WebElement> getAllWebElements(String locator, String locValue) {
		switch (locator) {
		case "id":
		...
		case "css":
			return driver.findElementsByCssSelector(locValue);
		case "xpath":
			return driver.findElementsByXPath(locValue);
		case "accessibilityId":
			return driver.findElementsByAccessibilityId(locValue);
		case "image":
			return driver.findElementsByImage(locValue);
		}
		return null;
	}
	...
	public boolean scrollFromDownToUpinAppUsingTouchActions() {
		try {
			...
			touch.add(new TouchAction<>(driver)
					...
					.moveTo(PointOption.point(x0, y0)).release())
			.perform();
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public boolean scrollFromUpToDowninAppUsingTouchActions() {
		try {
			...
			MultiTouchAction touch = new MultiTouchAction(driver);
			touch.add(new TouchAction<>(driver).press(PointOption.point(x1, y1))
					.waitAction(WaitOptions.waitOptions(Duration.ofSeconds(2)))
					.moveTo(PointOption.point(x0, y0))
					.release())
			.perform();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public boolean scrollFromLeftToRightInAppUsingTouchActions() {
		try {
			...
			int y0 = (int) (size.getHeight() * 0.5);
			MultiTouchAction touch = new MultiTouchAction(driver);
			touch.add(new TouchAction<>(driver).press(PointOption.point(x0, y0))
					.waitAction(WaitOptions.waitOptions(Duration.ofSeconds(2)))
	...
	public void pinchUsingTouchActions() {
		...
		tAction0
		.press(PointOption.point(source.x / 3, source.y / 3))
		.waitAction(WaitOptions.waitOptions(Duration.ofSeconds(1)))
		.moveTo(PointOption.point(source.x / 2, source.y / 2))
		.release();
		tAction1
		.press(PointOption.point(source.x * 3 / 4, source.y * 3 / 4))
		.waitAction(WaitOptions.waitOptions(Duration.ofSeconds(1)))
		.moveTo(PointOption.point(source.x / 2, source.y / 2))
		.release();
	...
	public void zoomUsingTouchActions() {
		...
		TouchAction<?> tAction1 = new TouchAction<>(driver);
		tAction0
		.press(PointOption.point(source.x / 2, source.y / 2))
		.waitAction(WaitOptions.waitOptions(Duration.ofSeconds(1)))
		.moveTo(PointOption.point(source.x / 3, source.y / 3))
		.release();
		tAction1
		.press(PointOption.point(source.x * 2, source.y * 2))
		.waitAction(WaitOptions.waitOptions(Duration.ofSeconds(1)))
		.moveTo(PointOption.point(source.x * 3 / 4, source.y * 3 / 4))
		.release();
	...
	public List<WebElement> getNotesItem()
	{
